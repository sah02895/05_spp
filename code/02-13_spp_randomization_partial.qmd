...
**There are 12 rows in the resulting object. The number of rows in this case are a function of having 4 block with 3 whole plot treatments and no consideration for the split plot treatments (3 treatments x 4 blocks = 12 whole plots).**
...
**The object contains 36 rows. The number of rows for this object is a function of each whole plot now containing three split plots (4 blocks x 3 whole plot treatments x 3 split plot treatments = 36 split plots).**
...
**Values in wplot are duplicated three times to account for the three split plots now present within each whole plot. Each splot value represents an individual split plot.**
...
ggplot(data = spp_rcbd_layout, aes(x = row, 
  ...
  geom_tile(aes(color = factor(krate_kgha)),show.legend = F,size = 2,linetype = 2,fill = NA) +
