...
**There are 12 rows in the resulting object. The number of rows in this case are a function of each block having the 3 whole treatments (K rate) with no consideration for the split plot treatments (N rate) (3 treatments x 4 blocks = 12 whole plots).**
...
**Values in wplot are  duplicated three times, which are all the same whole plot, to account for the three-way split provided in splot which represents the individual split plots.**
